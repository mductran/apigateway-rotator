...
func (ag *apiGateway) Initialize(region string, errChan chan error) {
	...
	if ApiExistsInRegion(client, ag.Name, region) {
		errChan <- fmt.Errorf("an API already exists with name: %s in region %s", ag.Name, region)
		return
	}
	...
	if err != nil {
		errChan <- fmt.Errorf("cannot create new API: %w", err)
		return
	}
	...
	if err != nil {
		errChan <- fmt.Errorf("cannot create method: %w", err)
		return
	}
	...
	if err != nil {
		errChan <- fmt.Errorf("cannot create integration: %w", err)
		return
	}

	wildcardPath := "{proxy+}"
	wildcardHandler, err := client.CreateResource(context.TODO(), &apigateway.CreateResourceInput{
		RestApiId: newApi.Id,
		ParentId:  newApi.RootResourceId,
		PathPart:  &wildcardPath,
	})
	if err != nil {
		errChan <- fmt.Errorf("cannot create wildcard handler: %w", err)
		return
	}
	...
	if err != nil {
		errChan <- fmt.Errorf("cannot create wildcard method input: %w", err)
		return
	}
	...
	if err != nil {
		errChan <- fmt.Errorf("cannot integrate wildcard method: %w", err)
		return
	}
...
func main() {
	...
	for _, re := range DEFAULT_REGIONS {
		go gateway.Initialize(re, errsChan)
	}
